(()=>{
  "use strict";
  if(window.MAOrderView?.version==="1")return;
  ...
  function render(data){
    ...
    if(title)title.textContent=`Заказ №${r.order_no}`;if(status)status.textContent=(STATUS[r.status]||r.status||"ЗАКАЗ").toUpperCase();if(sub)sub.textContent=`${r.service||""} · ${[r.device,r.model].filter(Boolean).join(" ")}`;if(!body)return;
  ...
  window.MAOrderView={version:"1",render};
})();
