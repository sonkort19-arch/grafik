(()=>{
  ...
  const DATETIME_FMT=new Intl.DateTimeFormat("ru-RU",{timeZone:"Europe/Moscow",day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"});
  const $=id=>document.getElementById(id);
  const esc=value=>String(value??"").replace(/[&<>"']/g,ch=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[ch]));
  const num=value=>{const n=Number(value);return Number.isFinite(n)?n:0;};
  const money=value=>MONEY_FMT.format(num(value))+" ₽";
  const dateTime=value=>{if(!value)return"—";const d=new Date(value);if(Number.isNaN(d.getTime()))return"—";return DATETIME_FMT.format(d);};
  ...
  function backgroundRefreshLists(){
    if(dashboardRefreshTimer)clearTimeout(dashboardRefreshTimer);
    const run=()=>{dashboardRefreshTimer=0;const btn=$("refreshDashboard");if(btn&&!btn.disabled)btn.click();};
    if("requestIdleCallback" in window){window.requestIdleCallback(run,{timeout:900});return;}
    dashboardRefreshTimer=setTimeout(run,260);
  }

  function patchStatusUi(data,status){
    const body=$("repairDetailBody"),buttons=body?.querySelectorAll(".status-stepper [data-set-status]")||[];
    buttons.forEach(btn=>btn.classList.toggle("current",btn.dataset.setStatus===status));
    const badge=$("repairDetailStatus");if(badge)badge.textContent=statusName(data,status).toUpperCase();
  }
  ...
  async function setStatus(id,status,button){
    ...
    try{
      const result=await api("set-status",{id,status});
      const controller=window.MAOrderController;
      const patch={...(result?.repair||{}),status};
      controller?.updateSnapshot?.(patch);
      patchStatusUi(controller?.current||{repair:patch},status);
      toast(`Статус: ${statusName(controller?.current,status)}`);
      backgroundRefreshLists();
      setTimeout(()=>controller?.refresh?.({reason:"status",keepMarkup:true}).catch?.(()=>{}),120);
  ...
  function openerFromEvent(event){if(event.target.closest?.("#repairDetailOverlay"))return null;return event.target.closest?.("[data-repair-id],[data-recent-repair]")||null;}
  function prefetchOpen(event){const opener=openerFromEvent(event);if(!opener)return;const id=opener.dataset.repairId||opener.dataset.recentRepair;if(id)window.MAOrderController?.prefetch?.(String(id)).catch(()=>{});}
  function interceptOpen(event){
    const opener=openerFromEvent(event);if(!opener)return;
  ...
  document.addEventListener("pointerdown",prefetchOpen,{capture:true,passive:true});
  document.addEventListener("touchstart",prefetchOpen,{capture:true,passive:true});
  document.addEventListener("mouseover",prefetchOpen,{capture:true,passive:true});
