(()=>{
  ...
  function ensureHeader(){const head=document.querySelector("#repairDetailOverlay .order-modal-head");if(!head)return;let amount=head.querySelector(".hc-order-amount");if(!amount){amount=document.createElement("div");amount.className="hc-order-amount";head.querySelector("div")?.appendChild(amount);}const current=window.MAOrderController?.current,total=current?.paymentSummary?.total??current?.repair?.final_price??current?.repair?.estimated_price??0;setText(amount,money(total));}
