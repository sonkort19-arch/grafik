...
begin
  if p_phone_normalized is null or length(trim(p_phone_normalized)) < 10 then
    raise exception 'Проверь номер телефона';
  end if;
  if p_service is null or trim(p_service) = '' then
    raise exception 'Выбери точку';
  end if;
  if p_issue is null or trim(p_issue) = '' then
    raise exception 'Укажи неисправность';
  end if;
  if coalesce(p_estimated_price, 0) < 0 or coalesce(p_estimated_price, 0) > 100000000 then
    raise exception 'Неверная сумма';
  end if;
  if coalesce(p_warranty_days, 14) < 0 or coalesce(p_warranty_days, 14) > 730 then
    raise exception 'Гарантия должна быть от 0 до 730 дней';
  end if;
  if v_method not in ('cash','card','transfer','other') then
    raise exception 'Неверный способ оплаты';
  end if;
  if coalesce(p_initial_payment, 0) < 0 or coalesce(p_initial_payment, 0) > 100000000 then
    raise exception 'Неверная сумма предоплаты';
  end if;
  ...
  insert into public.ma_crm_repairs(
    customer_id, service, device, model, imei, issue,
    estimated_price, manager, master, status, comment,
    created_by, updated_by, due_at, warranty_days, warranty_note
  ) values (
    v_customer_id, trim(p_service), coalesce(trim(p_device), ''), coalesce(trim(p_model), ''),
    coalesce(trim(p_imei), ''), trim(p_issue), coalesce(p_estimated_price, 0),
    coalesce(trim(p_manager), ''), coalesce(trim(p_master), ''), 'accepted',
    coalesce(trim(p_comment), ''), coalesce(trim(p_actor), ''), coalesce(trim(p_actor), ''),
    p_due_at, coalesce(p_warranty_days, 14), coalesce(trim(p_warranty_note), '')
  ...
  if coalesce(p_initial_payment, 0) > 0 then
    ...
    insert into public.ma_crm_payments(
      repair_id, kind, method, amount, note, created_by, cashbox_id
    ) values (
      v_repair_id, 'payment', v_method, p_initial_payment,
      'Предоплата при приёме', coalesce(trim(p_actor), ''), v_cashbox_id
    );
  end if;
