-- Automatic percentage compensation must never turn a loss-making repair into a debt for staff.
-- Keep raw order profit for manager economics, but clamp automatic percentage bases at zero.
...
begin
  ...
  loop
    ...
    v_total_profit := v_total_profit + v_raw_basis;
    v_basis := greatest(v_raw_basis, 0);
  ...
  if btrim(coalesce(v_repair.manager, '')) <> '' then
    ...
    v_percent := coalesce(v_comp_percent, 0);
    v_manager_basis := greatest(round(v_total_profit, 2), 0);
    v_amount := round((v_manager_basis * v_percent / 100)::numeric, 2);
    ...
    insert into public.ma_crm_payroll_entries(
      ...
      v_amount,
      'issued',
      jsonb_build_object('repairPercent', v_percent, 'rawProfit', round(v_total_profit, 2)),
      left(coalesce(p_actor, ''), 100)
    )
    on conflict(source_type, source_id, employee, entry_type) do nothing;
  ...
  return jsonb_build_object(
    ...
    'payable_profit', greatest(round(v_total_profit, 2), 0)
  );
end;
$$;
