...
async function detail(req:Request,body:any){
  ...
  const statusQuery=db.from("ma_crm_status_definitions").select("code,name,group_code,sort_order,actions,system").eq("active",true).order("sort_order");
...
async function commerce(req:Request,body:any){const actor=await requireAuth(req),repair=await repairFor(actor,body.id||body.repairId),data=await totals(repair);return {ok:true,repair:{id:repair.id,service:repair.service,estimated_price:repair.estimated_price,final_price:repair.final_price,warranty_days:repair.warranty_days,warranty_note:repair.warranty_note,due_at:repair.due_at,issued_at:repair.issued_at},...data};}
...
async function upsertItem(req:Request,body:any){
  const actor=await requireAuth(req),repair=await repairFor(actor,body.repairId),id=text(body.id,80),itemType=text(body.itemType,20),title=text(body.title,240),inventoryProductId=text(body.inventoryProductId,80)||null,catalogId=text(body.serviceCatalogId,80)||null;
  if(!["service","part"].includes(itemType))throw new Error("Выбери тип позиции");
  const quantity=positive(body.quantity,10000),unitPrice=decimal(body.unitPrice),unitCost=decimal(body.unitCost),discount=decimal(body.discount||0),executor=text(body.executor,100),key=text(body.idempotencyKey,160)||crypto.randomUUID();
  if(discount>quantity*unitPrice)throw new Error("Скидка не может быть больше стоимости позиции");

  if(!id){
    const {data,error}=await db.rpc("ma_crm_add_order_item",{
      p_repair_id:repair.id,p_item_type:itemType,p_title:title,p_quantity:quantity,p_unit_price:unitPrice,p_unit_cost:unitCost,
      p_discount_amount:discount,p_executor:executor,p_service_catalog_id:catalogId,p_inventory_product_id:inventoryProductId,
      p_actor:actor.employee,p_idempotency_key:key,
    });
    if(error)throw error;
    const item=data?.item||null,finalPrice=data?.finalPrice??data?.finalprice??null,summary=await totals({...repair,final_price:finalPrice});
    return {ok:true,item,duplicate:!!data?.duplicate,idempotencyKey:key,finalPrice,...summary};
  }

  const {data:old,error:oldError}=await db.from("ma_crm_repair_items").select("*").eq("id",id).eq("repair_id",repair.id).single();
  if(oldError||!old)throw new Error("Позиция не найдена");
  if(old.inventory_product_id)throw new Error("Складскую запчасть удалите и добавьте заново");
  if(!title)throw new Error("Укажи название позиции");
  const payload:any={item_type:itemType,display_type:itemType,title,quantity,unit_price:unitPrice,unit_cost:unitCost,discount_amount:discount,executor,service_catalog_id:catalogId,updated_at:new Date().toISOString()};
  const {data:item,error}=await db.from("ma_crm_repair_items").update(payload).eq("id",id).eq("repair_id",repair.id).select("*").single();if(error)throw error;
  const finalPrice=await recalcRepair(repair.id,actor.employee);await logEvent(repair.id,"item_updated",actor.employee,old,item,`Изменена позиция «${item.title}»`,'item-update:'+item.id+':'+crypto.randomUUID());
  const summary=await totals({...repair,final_price:finalPrice});return {ok:true,item,duplicate:false,finalPrice,...summary};
}
...
async function deleteItem(req:Request,body:any){
  ...
  const finalPrice=await recalcRepair(repair.id,actor.employee);await logEvent(repair.id,"item_deleted",actor.employee,old,null,`Удалена позиция «${old.title}»`,'item-delete:'+old.id+':'+crypto.randomUUID());
  const summary=await totals({...repair,final_price:finalPrice});return {ok:true,finalPrice,...summary};
}
...
async function setStatus(req:Request,body:any){const actor=await requireAuth(req),repair=await repairFor(actor,body.id||body.repairId),status=text(body.status,80);const {data,error}=await db.rpc("ma_crm_set_repair_status",{p_repair_id:repair.id,p_status:status,p_actor:actor.employee});if(error)throw error;const updated=await repairFor(actor,repair.id);return {ok:true,result:data,repair:updated};}
...
async function updateOrder(req:Request,body:any){
  ...
  if("dueAt" in body){const raw=text(body.dueAt,80);if(raw&&Number.isNaN(new Date(raw).getTime()))throw new Error("Неверная дата срока");patch.due_at=raw?new Date(raw).toISOString():null;}if("warrantyDays" in body){const n=Number(body.warrantyDays);if(!Number.isInteger(n)||n<0||n>730)throw new Error("Гарантия должна быть от 0 до 730 дней");patch.warranty_days=n;}if("warrantyNote" in body)patch.warranty_note=text(body.warrantyNote,1000);
