(()=>{
  ...
  function prunePrefetches(){const t=now();for(const [key,entry] of prefetches){if(t-entry.at>PREFETCH_TTL*2)prefetches.delete(key);}}
  ...
  function prefetch(id){
    const repairId=String(id||"").trim();if(!repairId)return Promise.resolve(null);
    prunePrefetches();
    const existing=prefetches.get(repairId);
    if(existing&&now()-existing.at<PREFETCH_TTL)return existing.promise;
    const at=now();
    const promise=readRepair(repairId).then(data=>{const entry=prefetches.get(repairId);if(entry&&entry.promise===promise){entry.data=data;entry.at=now();}return data;}).catch(error=>{const entry=prefetches.get(repairId);if(entry?.promise===promise)prefetches.delete(repairId);throw error;});
    prefetches.set(repairId,{at,promise,data:null});
    return promise;
  }

  function prefetched(repairId){const entry=prefetches.get(repairId);if(!entry)return null;if(now()-entry.at>PREFETCH_TTL){prefetches.delete(repairId);return null;}return entry.promise;}
  function invalidatePrefetch(id){if(id)prefetches.delete(String(id));else prefetches.clear();}

  function markReady(token,data,attempt=0){
    ...
    if(body?.querySelector(".order-layout")){
      ...
      state.readyToken=token;body.dataset.orderRepairId=state.repairId;body.dataset.orderToken=String(token);emit("ma:order:ready",{data,repair:data?.repair||null,body});return;
    }
    if(attempt>=60){const message="Карточка заказа не успела отрисоваться";state.error=message;emit("ma:order:error",{message});return;}
    requestAnimationFrame(()=>markReady(token,data,attempt+1));
  }
  ...
  async function open(id,options={}){
    ...
    try{
      const cached=(options.reason||"open")==="open"?prefetched(repairId):null;
      const data=await (cached||readRepair(repairId,{signal:controller.signal}));
      ...
      markReady(token,data);
      return data;
    }
    catch(error){if(error?.name==="AbortError"||controller.signal.aborted||token!==state.token)return null;state.request=null;state.loading=false;state.error=error?.message||"Не удалось открыть заказ";const status=Number(error?.status);if((status===401||status===403)&&typeof state.authErrorHandler==="function")state.authErrorHandler(state.error);else setError(state.error);emit("ma:order:error",{message:state.error,error});return null;}
  }

  async function refresh(options={}){if(!state.repairId||state.closed)return null;invalidatePrefetch(state.repairId);return open(state.repairId,{...options,reason:options.reason||"refresh",keepMarkup:options.keepMarkup===true});}
  ...
  function updateSnapshot(patch){if(!state.data?.repair)return;state.data={...state.data,repair:{...state.data.repair,...patch}};invalidatePrefetch(state.repairId);emit("ma:order:changed",{data:state.data,repair:state.data.repair,patch});}
  function replaceSnapshot(data){if(!data?.repair)return;state.data=data;state.repairId=String(data.repair.id||state.repairId);invalidatePrefetch(state.repairId);emit("ma:order:changed",{data,repair:data.repair,replace:true});}
